...
export class SupportCardGenerator {
    ...
    static async createSupportCard(card: any, position: Vector2d = new Vector2d(0, 0), indexCount: number = 0): Promise<THREE.Group> {
        ...
        await this.addOptionalSupportTextures(card, cardGroup, cardWidth, cardHeight, position, indexCount);
    ...
    private static async addOptionalSupportTextures(
        card: any,
        group: THREE.Group,
        cardWidth: number,
        cardHeight: number,
        ...
        if (cardKindsTexture) {
            ...
            const supportPosition = new Vector2d(position.getX() + cardWidth * 0.5, position.getY() - cardHeight * 0.5);
            this.addTextureToGroup(group, cardKindsTexture, kindsWidth, kindsHeight, supportPosition, 'kindsTextureId', indexCount);
        }
        ...
        if (cardRaceTexture) {
            ...
            const additionalPosition = new Vector2d(position.getX() + cardWidth * 0.5, position.getY() + cardHeight * 0.5);
            this.addTextureToGroup(group, cardRaceTexture, raceWidth, raceHeight, additionalPosition, 'raceTextureId', indexCount);
        }
    }
