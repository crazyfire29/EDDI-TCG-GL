...
export class CardGenerationHandler {
    static cardKindHandlers: { [key in CardKind]?: (card: any, positionVector: Vector2d) => void } = {
    ...
    static createCardById(cardId: number, positionVector: Vector2d) {
        ...
        if (card) {
            ...
            if (handler) {
                return handler(card, positionVector);
            } else {
