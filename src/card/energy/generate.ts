...
export class EnergyCardGenerator {
    ...
    static async createEnergyCard(card: any, position: Vector2d = new Vector2d(0, 0), cardIndex: number = 0): Promise<THREE.Group> {
        const textureManager = TextureManager.getInstance();
        console.log('createEnergyCard() cardIndex:', cardIndex);
        ...
        const cardWidth = 0.064935 * currentWidth;
        ...
        this.saveInitialPosition(mainCardMesh, position, cardWidth, cardHeight, 'mainCardTextureId', cardIndex);

        await this.addOptionalItemTextures(card, cardGroup, cardWidth, cardHeight, position, cardIndex);
    ...
    private static async addOptionalItemTextures(card: any, group: THREE.Group, cardWidth: number, cardHeight: number, position: Vector2d, cardIndex: number) {
        const textureManager = TextureManager.getInstance();
        ...
        if (cardKindsTexture) {
            ...
            this.addTextureToGroup(group, cardKindsTexture, kindsWidth, kindsHeight, kindsPosition, 'kindsTextureId', cardIndex);
        }
        ...
        if (cardRaceTexture) {
            ...
            this.addTextureToGroup(group, cardRaceTexture, raceWidth, raceHeight, racePosition, 'raceTextureId', cardIndex);
        }
    }

    private static addTextureToGroup(group: THREE.Group, texture: THREE.Texture, width: number, height: number, position: Vector2d, textureId: string, cardIndex: number) {
        const mesh = MeshGenerator.createMesh(texture, width, height, position);
        mesh.userData.textureId = textureId;
        group.add(mesh);
        this.saveInitialPosition(mesh, position, width, height, textureId, cardIndex);
    }

    private static saveInitialPosition(mesh: THREE.Mesh, position: Vector2d, width: number, height: number, textureId: string, cardIndex: number) {
